...
import Output2 from "../../assets/blogs/using-bun-or-deno-as-a-web-server-in-tauri/output-2.png"
...
import ProcessFlowV2 from "../../assets/blogs/using-bun-or-deno-as-a-web-server-in-tauri/process-v2.png"
import HTTPAuth from "../../assets/blogs/using-bun-or-deno-as-a-web-server-in-tauri/http-auth.png"

> <p class="text-purple-500 m-0! p-0!">IMPORTANT</p>
> <p>I've found a better way to communicate between Bun server and Tauri client using bi-directional RPC system that requires zero extra Rust code and also completely eliminates the requirement for authorization for security. It is available on branch [v2](https://github.com/niraj-khatiwada/tauri-bun/tree/v2) in GitHub Repo. Please skip to [this](#v2) section to read the newer implementation detail.</p>
...
Recently, I discovered a brand new RPC library called [kkrpc](https://docs.kkrpc.kunkun.sh/) that handles RPC connection across various environments like `http`, `websocket`, `stdio`, `web worker`, `iframe`, etc. It also has a first-class support for [Tauri](https://docs.kkrpc.kunkun.sh/examples/tauri/).
With this new setup, we don't even need any extra Rust code at all and also eliminates the need for authorization, compared to what we did in earlier setup.
Furthermore, it is bi-directional so it just acts like a WebSocket server. It is very easy to setup and results in a fully typed bi-directional APIs between server and client.
<Image src={ProcessFlowV2} alt="ProcessFlowV2" style="object-fit: contain; margin: 0 auto;" />

### Server setup:
<p class="m-0! p-0!">We'll first create some basic APIs and initialize our RPC channel in our server first.</p>
<code>
```ts
...
<p class="m-0! p-0!">Here, we've initialized our bi-directional Bun RPC server via Bun stdin/stdout stream as a channel. The `ServerApi` and `ClientApi` are just server and client API interfaces for type hints. I've created some simple server APIs for this demo.</p>
<code>
```ts
import type { API as ServerApi } from '../types/api'
...
### Client Setup:
<p class="m-0! p-0!">Now, let's setup the RPC connection in our Tauri webview client. We'll initiate it in our root `router.tsx` to ensure that the RPC connection is live before our frontend mounts.</p>
<code>
```ts
// router.tsx
...
<p class="m-0! py-0!">Here're we've initialized our RPC connection with our Bun RPC channel via Tauri Sidecar. After the connection is established over the bi-directional channel, we're passing the api instance `serverApi` as a global router context.
Now, we can use this RPC connection to call our server API anywhere in our React app.</p>
<code>
```
...
<p class="my-0!">Right now, Tauri webview can communicate with Rust backend and with Bun server as well. But, there's no direct way for Bun server to communicate with Rust backend.
But, there's a way to communicate with Rust at any point in Bun server via Tauri commands, thanks to bi-directional RPC channel.
Let's add a simple Tauri command.</p>
<code>
```rust
#[tauri::command]
...
<p class="my-0!">Now, let's add an API to call this command from Tauri webview client.</p>
<code>
```ts
...
<p class="my-0!">Finally, we can now call this client API easily at any point in our Bun server.</p>
<code>
```ts
...
<Image src={Output2} alt="Tauri Bun RPC" style="object-fit: contain; margin: 0 auto;" />
</section>
<br />

<p class="my-0!">Code: <i>Please switch to branch `v2` for RPC based setup. The old setup is still available on `main` branch.</i></p>
<br />
<p class="my-0!">[Tauri + Bun](https://github.com/niraj-khatiwada/tauri-bun) and [Tauri + Deno](https://github.com/niraj-khatiwada/tauri-deno)</p>
